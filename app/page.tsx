...
export default function Home() {
  ...
  // Horizontal resize between left/right panes
  function handleHorizResizeStart(e: React.MouseEvent) {
    e.preventDefault();
  ...
  return (
    <div className="h-full flex flex-col bg-background">
      ...
      <header className="flex items-center justify-between px-6 py-3 border-b border-border/60 shrink-0">
        <span className="text-base font-semibold tracking-tight text-foreground/80 select-none">
          Write⍤mal
        </span>
        ...
        <div className="flex items-center gap-1 bg-muted rounded-full p-0.5">
          {LANGUAGES.map((lang) => (
            <button
              ...
              className={`text-xs px-4 py-1.5 rounded-full font-medium transition-all duration-200 cursor-pointer ${
                language === lang
      ...
      <main ref={mainRef} className="flex-1 flex overflow-hidden min-h-0">
        {/* Left — editor */}
        <div
          className="flex flex-col border-r border-border/60 relative min-w-0 shrink-0"
        ...
        >
          ...
          {hasCorrections && (
            <div className="absolute bottom-5 right-4 flex items-center gap-2">
              ...
              <button
                ...
                className="text-xs px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-all duration-150 font-medium cursor-pointer"
              >
                Apply corrections
              </button>
            </div>
          ...
          {/* Mascot walks within this container */}
          <div className="absolute bottom-0 left-0 right-0 h-[110px] pointer-events-none">
        ...
        <div
          className="w-1.5 shrink-0 cursor-col-resize group flex items-center justify-center hover:bg-muted/60 transition-colors z-20"
        ...
        >
          {/* Correction panel fills remaining space */}
          ...
          {/* Drag handle */}
          <div
            className="h-2.5 shrink-0 cursor-row-resize group flex items-center justify-center border-t border-border/60 hover:border-border transition-colors"
          ...
          {/* Assist panel — height controlled by drag */}
          <div
            className="shrink-0 overflow-hidden flex flex-col"
            style={{ height: `${assistHeightPct}%` }}
          >
            <AssistPanel
